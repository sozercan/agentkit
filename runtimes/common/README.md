...
- `runtime.py` — the **neutral run contract**: `RunResult`, `AgentRunError`, and
  the `RuntimeFactory` protocol each adapter implements.
...
server uses only `factory.build_agent(spec)` and
`factory.run_agent(agent, prompt, history) -> RunResult`, so it imports **no**
agent framework. This is what keeps the lock-in boundary (plan §12) confined to
...
1. `agent_factory.py` implementing `build_agent` + `run_agent` (the only file that
   imports the framework), and
